...
/// plus `_query_index: Int32` and `_distance: Float32`, containing at most `k`
/// rows per query vector sorted by `(_query_index ASC, _distance ASC)`.
///
/// The caller can chain additional DataFrame operations before collecting.
///
/// Set `with_row_id` to include the `_rowid` meta column in the output.
pub async fn batch_knn(
    ...
    let mut adapter = BaseTableAdapter::try_new(table).await?;
    if with_row_id {
        adapter = adapter.with_row_id();
    }
