...
use lance_io::object_store::{
    DirEntry, ReadDirOptions, StorageOptionsAccessor, StorageOptionsProvider,
};
...
fn table_name(entry: &DirEntry, dir_suffix: &str) -> Option<String> {
    entry
        .is_dir()
        .then(|| entry.name.strip_suffix(dir_suffix))
        .flatten()
        .map(String::from)
}
    ...
    async fn list_tables(&self, request: ListTablesRequest) -> Result<ListTablesResponse> {
        ...
        loop {
            ...
            let listing = self
                ...
                .await?;
            page_token = listing.next_token;
            tables.extend(
                listing
                    .values
                    .iter()
                    .filter_map(|entry| table_name(entry, &dir_suffix)),
            );
...
mod tests {
    ...
    async fn test_the_page_token_is_not_a_table_name() {
        ...
        // And a token from nowhere is refused rather than read as a position.
        assert!(
            db.list_tables(ListTablesRequest {
                page_token: Some("a".to_string()),
                ..Default::default()
            })
            .await
            .is_err()
