...
use datafusion_sql::sqlparser::ast::{
    Expr, JoinOperator, LimitClause, ObjectNamePart, Query, SelectItem, SetExpr, Statement,
...
use super::{MaterializedViewDefinition, ViewProjection, ViewUnnest};
...
const SHAPE: &str = "a materialized view is defined by \
    `SELECT columns FROM table [, UNNEST(column) AS alias] [WHERE predicate] [LIMIT n]`";
...
fn normalize_from(mut query: Query) -> Query {
    if let SetExpr::Select(select) = query.body.as_mut() {
        if select.from.len() == 1
            ...
            && matches!(select.from[0].joins[0].relation, TableFactor::UNNEST { .. })
        {
        ...
        // `UNNEST(c) e` and `UNNEST(c) AS e` are one alias.
        if let Some(TableWithJoins {
            relation: TableFactor::UNNEST {
                alias: Some(alias), ..
            },
            ..
        }) = select.from.get_mut(1)
        {
            alias.explicit = true;
        }
    }
...
fn extract(query: &Query) -> Result<MaterializedViewDefinition> {
    ...
    let unnest = match from.next().map(|f| &f.relation) {
        None => None,
...
pub fn render(definition: &MaterializedViewDefinition) -> String {
    ...
    if let Some(unnest) = &definition.unnest {
        sql.push_str(&format!(
            ", UNNEST({}) AS {}",
            ident_sql(&unnest.column),
            ident_sql(&unnest.alias)
        ));
    }
...
mod tests {
    ...
    fn parsed_parts_are_the_relational_shape() {
        ...
        assert!(parse("SELECT * FROM t").unwrap().projections.is_empty());
    }
