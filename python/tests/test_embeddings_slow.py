...
def test_basic_text_embeddings(alias, tmp_path):
    ...
    query = "greetings"
    actual = table.search(query, vector_column_name="vector") \
        .limit(1) \
        .to_pydantic(Words)[0]

    vec = func.compute_query_embeddings(query)[0]
    expected = table.search(vec, vector_column_name="vector") \
        .limit(1) \
        .to_pydantic(Words)[0]
    ...
    actual = (
        table.search(query, vector_column_name="vector2") \
            .limit(1) \
            .to_pydantic(Words)[0]
    )
    assert actual.text != "hello world"
...
def test_openclip(tmp_path):
    ...
    # text search
    actual = table.search("man's best friend", vector_column_name="vector") \
        .limit(1) \
    ...
    actual = table.search(query_image, vector_column_name="vector") \
        .limit(1) \
        .to_pydantic(Images)[0]
    assert actual.label == "dog"
    other = (
