...
class PyExpr:
    ...
    def between(self, low: "PyExpr", high: "PyExpr") -> "PyExpr": ...
...
def aggregate_sum(expr: PyExpr) -> PyExpr: ...
def aggregate_avg(expr: PyExpr) -> PyExpr: ...
def aggregate_min(expr: PyExpr) -> PyExpr: ...
def aggregate_max(expr: PyExpr) -> PyExpr: ...
def aggregate_count(expr: PyExpr) -> PyExpr: ...

class NativeDataFrame:
    @staticmethod
    def from_table(name: str, schema: pa.Schema) -> "NativeDataFrame": ...
    def select(self, expressions: List[PyExpr]) -> "NativeDataFrame": ...
    def filter(self, predicate: PyExpr) -> "NativeDataFrame": ...
    def aggregate(
        self, groups: List[PyExpr], aggregates: List[PyExpr]
    ) -> "NativeDataFrame": ...
    def sort(
        self, expressions: List[Tuple[PyExpr, bool, bool]]
    ) -> "NativeDataFrame": ...
    def limit(self, count: int, offset: int = 0) -> "NativeDataFrame": ...
    def distinct(self) -> "NativeDataFrame": ...
    def alias(self, name: str) -> "NativeDataFrame": ...
    def column(self, name: str) -> PyExpr: ...
    def with_column(self, name: str, expression: PyExpr) -> "NativeDataFrame": ...
    def drop(self, columns: List[str]) -> "NativeDataFrame": ...
    def with_column_renamed(
        self, old_name: str, new_name: str
    ) -> "NativeDataFrame": ...
    def join(
        self,
        other: "NativeDataFrame",
        left_on: List[str],
        right_on: List[str],
        how: str = "inner",
    ) -> "NativeDataFrame": ...
    def union(
        self, other: "NativeDataFrame", all: bool = True
    ) -> "NativeDataFrame": ...
    def intersect(
        self, other: "NativeDataFrame", all: bool = False
    ) -> "NativeDataFrame": ...
    def except_(
        self, other: "NativeDataFrame", all: bool = False
    ) -> "NativeDataFrame": ...
    ...
    def to_substrait_with_version(self) -> Tuple[bytes, str]: ...
