...
from ._lancedb import coerce_batch as _coerce_batch
from ._lancedb import write_schema_for as _write_schema_for
...
def _cast_to_target_schema(
    ...
    if not allow_subschema and len(reader.schema) != len(target_schema):
        raise ValueError(
            "Input table has different number of columns than target schema"
        )

    # The coercion itself is the Rust write path, the same one `add` runs, so a blob,
    # JSON or all-null column is built identically however it reaches Lance. Ask it
    # for the resulting schema rather than predicting it: that is what batches get.
    write_schema = _write_schema_for(reader.schema, target_schema)

    def gen():
        for batch in reader:
            yield _coerce_batch(batch, write_schema)

    return pa.RecordBatchReader.from_batches(write_schema, gen())
