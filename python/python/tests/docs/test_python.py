...
async def test_pandas_and_pyarrow_async():
    ...
    query_vector = [100, 100]
    # Pandas DataFrame
    df = await async_tbl.search(query_vector).limit(1).to_pandas()
    print(df)
    # --8<-- [end:vector_search_async]
    # --8<-- [start:vector_search_with_filter_async]
    # Apply the filter via LanceDB
    results = await async_tbl.search([100, 100]).where("price < 15").to_pandas()
    assert len(results) == 1
    assert results["item"].iloc[0] == "foo"

    # Apply the filter via Pandas
    df = results = await async_tbl.search([100, 100]).to_pandas()
    results = df[df.price < 15]
    assert len(results) == 1
    assert results["item"].iloc[0] == "foo"
...
async def test_polars_async():
    ...
    query = [3.0, 4.0]
    result = await table.search(query).limit(1).to_polars()
    print(result)
    print(type(result))
