...
@contextlib.contextmanager
def query_test_table(query_handler, *, server_version=semver.Version(0, 1, 0)):
    def handler(request):
        if request.path == "/v1/table/test/describe/":
            request.send_response(200)
            request.send_header("Content-Type", "application/json")
...
@pytest.mark.parametrize(
    "server_version", [semver.Version(0, 1, 0), semver.Version(0, 2, 0)]
)
def test_query_sync_batch_queries(server_version):
    def handler(body):
        ...
        vectors = body["vector"]
        if server_version >= semver.Version(
            0, 2, 0
