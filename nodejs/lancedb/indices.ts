...
/**
 * Options to create an `IVF_HNSW_FLAT` index
 */
export interface HnswFlatOptions {
  /**
   * The distance metric used to train the index.
   *
   * Default value is "l2".
...
export class Index {
   ...
   *
   */
  static hnswFlat(options?: Partial<HnswFlatOptions>) {
    return new Index(
      LanceDbIndex.hnswFlat(
        options?.distanceType,
        options?.numPartitions,
        options?.maxIterations,
        options?.sampleRate,
