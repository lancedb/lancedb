...
The process of inserting new data creates new files (representing data fragments) in Lance. Over time, if there are lots of small files, it presents an additional load to manage all of the metadata and performance goes down because you have to open multiple files to scan the dataset as opposed to reading from the same file. Compaction is required to maintain performance as you grow your dataset over time.
