...
sidebarTitle: "Query Plans"
description: "Read the plan LanceDB resolved for a query, and measure it with analyze."
icon: "map"
---
...
[`analyze_plan`](/search/explain), which executes it and annotates every
operator with runtime metrics.

<Note>
The SQL equivalent is [`EXPLAIN`](/sql/explain).
</Note>
...
Reading the plan tells you the shape. `analyze_plan` executes both sub-queries
and labels them `Vector Search Plan:` and `FTS Search Plan:`, which is how you
see whether the filter pushed into both halves uniformly and which half
dominates latency.
...
Where [`explain_plan`](#inspect-a-query-plan) tells you what a query intends to do,
this tells you what it cost.

<Note>
The SQL equivalent is [`EXPLAIN ANALYZE`](/sql/explain).
</Note>
